...
COMPILER="nvhpc" \
OPT="-O3 -Ofast -g" \
LDFLAGS="-g -L${TACC_CUDA_LIB} -L${TACC_NVMATH_LIB} -L${LIBRARY_PATH} -lcuda -lnvidia-ml" \
CGEOM="-DFIX_NODE_GEOM -DFIX_IONODE_GEOM" \
KSCGMULTI="-DKS_MULTICG=HYBRID " \
CTIME="-DNERSC_TIME -DCGTIME -DFFTIME -DFLTIME -DGATIME -DGFTIME -DREMAP -DPRTIME -DIOTIME -DGS_TIME" \
MY_CC=mpicc \
MY_CXX=mpicxx \
CUDA_HOME=${PATH_TO_CUDA} \
CUDA_COMP=${TACC_NVMATH_DIR} \
